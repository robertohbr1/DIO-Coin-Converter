...
class HistoryListAdapter : ListAdapter<ExchangeResponseValue, HistoryListAdapter.ViewHolder>(DiffCallback()) {
    ...
    inner class ViewHolder(
        ...
        fun bind(item: ExchangeResponseValue) {
            ...
            val coin1 = Coin.getByName(item.code)
            val coin2 = Coin.getByName(item.codein)
            binding.tvValue.text = item.qtt.formatCurrency(coin1.locale) + " * " + item.bid.formatCurrency(coin1.locale) + " = " + item.total.formatCurrency(coin2.locale)
        }
    }
}
